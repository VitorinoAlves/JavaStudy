package br.com.calves.controller;

import br.com.calves.domain.Produto;
import br.com.calves.usecase.BuscaProduto;
import br.com.calves.usecase.CadastroProduto;
...
public class ProdutoResource {
    ...
    @Operation(summary = "Buscar uma lista paginada de produtos por status")
    public ResponseEntity<Page<Produto>> buscarPorStatus(Pageable pageable, @PathVariable(value = "status", required = true) Produto.Status status) {
    ...
    public ResponseEntity<Produto> atualizar(@RequestBody @Valid Produto produto) {
        return ResponseEntity.ok(cadastroProduto.atualizer(produto));
    }

    @DeleteMapping(value = "/{id}")
    @Operation(summary = "Remove um produto pelo seu identificador")
